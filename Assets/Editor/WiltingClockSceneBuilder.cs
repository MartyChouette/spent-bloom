...
/// <summary>
/// Builds the wilting_clock scene: one flower with leaves and petals,
/// ScissorStation, CuttingPlaneController, WiltingClock + WiltingClockHUD,
/// and a FlowerGradingUI for end-of-session results.
/// Menu: Window > Iris > Build Wilting Clock Scene
/// </summary>
public static class WiltingClockSceneBuilder
{
    ...
    public static void Build()
    {
        ...
        // ── 4. Flower (6 leaves, 4 petals for visible wilt progression)
        float stemHeight = 1.0f;
        var flowerResult = BuildFlower("WiltFlower", stemHeight, 6, 4,
        ...
        // ── 6. Managers: WiltingClock + WiltingClockHUD ───────────────
        var managersGO = new GameObject("Managers");
        var wiltingClock = managersGO.AddComponent<WiltingClock>();
        var wiltingHUD = managersGO.AddComponent<WiltingClockHUD>();

        // Wire WiltingClock
        var wcSO = new SerializedObject(wiltingClock);
        wcSO.FindProperty("brain").objectReferenceValue = flowerResult.brain;
        wcSO.ApplyModifiedPropertiesWithoutUndo();

        // Wire WiltingClockHUD
        var hudSO = new SerializedObject(wiltingHUD);
        hudSO.FindProperty("clock").objectReferenceValue = wiltingClock;
        hudSO.FindProperty("brain").objectReferenceValue = flowerResult.brain;

        // ── 7. Screen-space canvas with timer + status labels ─────────
        var uiCanvas = CreateScreenCanvas("UI_Canvas");

        // Timer label — top-center
        var timerLabel = CreateLabel("TimerLabel", uiCanvas.transform,
            new Vector2(0f, 200f), new Vector2(300f, 40f),
            "Next wilt: 5.0s", 24f, TextAlignmentOptions.Center);

        // Status label — top-left
        var statusLabel = CreateLabel("StatusLabel", uiCanvas.transform,
            new Vector2(-50f, 180f), new Vector2(300f, 40f),
            "Healthy: 10/10 parts", 20f, TextAlignmentOptions.Left);
        // Shift anchor toward top-left
        var statusRT = statusLabel.GetComponent<RectTransform>();
        statusRT.anchorMin = new Vector2(0.1f, 0.9f);
        statusRT.anchorMax = new Vector2(0.1f, 0.9f);

        // Wire HUD label references
        hudSO.FindProperty("timerLabel").objectReferenceValue = timerLabel.GetComponent<TMP_Text>();
        hudSO.FindProperty("statusLabel").objectReferenceValue = statusLabel.GetComponent<TMP_Text>();
        ...
        string path = $"{dir}/wilting_clock.unity";
        EditorSceneManager.SaveScene(scene, path);
        Debug.Log($"[WiltingClockSceneBuilder] Scene saved to {path}");
    }
